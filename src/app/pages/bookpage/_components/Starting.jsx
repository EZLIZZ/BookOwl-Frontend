...
export default function Starting({ data }) {
  ...
  return (
    <div className="relative p-6 max-w-5xl mx-auto bg-white shadow flex flex-row gap-3 sm:gap-8 mt-20">
      ...
      <div className="flex flex-col items-center w-1/3 sm:w-1/2" data-aos="fade-right">
        <img
      ...
      <div className="flex flex-col mt-50 sm:mt-24 lg:mt-3 w-2/3 lg:w-1/2" data-aos="fade-left">
        <p className="text-gray-500 text-md sm:text-lg italic">By {data.author.authorName}</p>
        <h1 className="text-xl sm:text-2xl font-bold text-[#b68a6a] mb-2">{data.bookName}</h1>

        <div className="flex items-center mb-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star
              key={i}
              className={`h-5 sm:h-6 w-5 sm:w-6 ${i < Math.floor(rating) || (i < rating && rating % 1 !== 0) ? "text-yellow-500" : "text-gray-300"}`}
        ...
        <p className="text-md sm:text-lg font-semibold text-green-600 mb-2">Price: Rs {data.price}</p>
